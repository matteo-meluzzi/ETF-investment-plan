...
pub async fn search_etf_isin(isin: Isin) -> Result<Vec<ETF>, YahooError> {
    ...
    let resp = provider.search_ticker(&isin.0).await?;

    Ok(resp.quotes.into_iter().map(|quote| {
        ETF::new(quote.long_name, isin.clone(), Ticker(quote.symbol))
    }).collect())
}

pub async fn get_price_of(ticker: &Ticker) -> Result<f64, YahooError> {
    let provider = yahoo::YahooConnector::new()?;
    let response = provider.get_latest_quotes(&ticker.0, "1d").await?;
...
mod tests {
    ...
    async fn test_search_etf_isin() {
        let xs = search_etf_isin(Isin("IE00B3ZW0K18".to_string())).await.unwrap();
        assert_eq!(xs.len(), 1);
    ...
    async fn test_get_price_of() {
        let price = get_price_of(&Ticker("IUSE.L".into())).await.unwrap();
        assert!(price > 0.0);
    }
}
